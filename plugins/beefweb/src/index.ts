...
import type { DefinePluginFn, PluginContext, CreateContext, RegisterOptions } from "mediafuse";
...
function readConfig(config: Record<string, unknown>) {
  return {
    channel: (config.channel as string) || "default",
    base: (config.beefwebBase as string) || DEFAULT_BASE,
    columns: (config.beefwebColumns as string[]) || DEFAULT_COLUMNS,
    pollMs: (config.beefwebPollMs as number) || DEFAULT_POLL_MS,
    displaySeconds: (config.beefwebDisplaySeconds as number) || DEFAULT_DISPLAY_SECONDS,
  };
}

function setup({ register: reg }: PluginContext): void {
  let dataStop: (() => void) | null = null;
    ...
    onCreate(ctx: CreateContext) {
      ...
      async function poll(): Promise<void> {
        ...
        try {
          ...
          const [artist, title] = activeItem.columns as string[];
...
function setupDashboard(container: HTMLDivElement, base: string, pollMs: number): void {
  ...
  function renderNowPlaying(): void {
    ...
    const pct = currentTrack.duration > 0
    ...
    const bar = nowPlaying.querySelector("[data-action=seek]") as HTMLDivElement | null;
    if (bar) {
  ...
  async function loadPlaylist(): Promise<void> {
    try {
      ...
      if (!res.ok) return;
      const data = await res.json();
      const active = data.playlists?.find((p: { isCurrent: boolean }) => p.isCurrent);
      if (!active) return;
...
export default (definePlugin: DefinePluginFn) => definePlugin("beefweb", setup);
