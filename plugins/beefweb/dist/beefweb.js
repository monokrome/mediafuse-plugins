...
function setup({ register: reg }) {
  ...
  reg("data", {
    onCreate(ctx) {
      ...
      beefwebBase = ctx.config.beefwebBase || DEFAULT_BASE;
      ...
      beefwebUrl = `${beefwebBase}/api/player?columns=${columnsParam}`;
      start();
    },
    onDestroy: stop
  });
  reg("dashboard", {
    onCreate(ctx) {
      ...
      dashboardUpdate = buildDashboardUI(ctx.container, beefwebBase);
    },
    onDestroy() {
  ...
  function buildDashboardUI(container, base) {
    container.innerHTML = "";
